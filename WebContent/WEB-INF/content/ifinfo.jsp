...
<s:form action="modifyInfoAction" class="if" id="infoform" >
	...
	<s:if test="#session.userinfo!=null&&(userId==null||userId==#session.userinfo.id)">
		...
		<input class="input" id="name" name="name" placeholder="30字符以内" value='<s:property value="userAccount.name" />' >
	</s:if>
	<s:else>
	...
	<s:if test="#session.userinfo!=null&&(userId==null||userId==#session.userinfo.id)">
		...
		<textarea class="input" id="sign" name="sign" placeholder="100字符以内" ><s:property value="userAccount.sign" /></textarea>
	</s:if>
	<s:else>
		<div class="text">ta的签名：</div>
		<s:if test="userAccount.sign!=null">
		<div class="input" id="sign"><s:property value="userAccount.sign" /></div>
		</s:if>
		<s:else>
	...
	<div class="container" id="sex">
		<div class="text" id="sex"><s:if test="#session.userinfo!=null&&(userId==null||userId==#session.userinfo.id)">性别：</s:if>
		    <s:else>ta的性别：</s:else></div>
	    <div class="button_group">
	        <input type="button" id="m" value="男" />
	        <input type="button" id="f" value="女" />
	        <input type="button" id="u" value="保密" />
	        <input type="text" name="sex" class="hide" hidden="true" value=<s:property value="userAccount.sex" /> />
	    </div>
	</div>
	<s:if test="userId==#session.userinfo.id||userId==null">
